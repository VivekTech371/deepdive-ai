    ...
    <div className="p-6 max-w-2xl">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <StatusCard
            label="Database"
            ok={health.database?.status === "connected" || health.database?.status === "ok"}
            detail={health.database?.latency ? `Latency: ${health.database.latency}ms` : undefined}
          />
          {health.memory && (
            <div className="border border-border rounded-lg bg-card p-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-mono">{(health.memory.rss / 1024 / 1024).toFixed(1)} MB</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">Heap Used</p>
                  <p className="font-mono">{(health.memory.heapUsed / 1024 / 1024).toFixed(1)} MB</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">Heap Total</p>
                  <p className="font-mono">{(health.memory.heapTotal / 1024 / 1024).toFixed(1)} MB</p>
                </div>
              </div>
            </div>
          )}
